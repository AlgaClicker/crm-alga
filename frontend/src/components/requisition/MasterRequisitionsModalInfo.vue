...
<template>
    <b-modal
        ...
        title="Редактировать уневерсальную заявку"
    ...
    >
                        ...
                        <table>
                            <tbody>
                                <tr v-for="item in masterRequisitionsMaterialInfoList" :key="item.id" >
                                    ...
                                    <td class="name-input"> {{ item.name }} </td>
                                    <td width="10%" > {{ item.unit.title }} </td>
        ...
        <template #modal-footer>
            <button 
                @click="sendRequisition(true)" class="mt-4 c-button-save--success" 
            >
                Отменить заявку
            </button>
        </template>
