...
<template>
    <b-modal
        id="directory-contracts-modal-add"
        ref="directory-contracts-modal-add"
        centered 
        content-class="c-modal-default c-modal-supply-requisition-add-invoice"
        @shown="mountedData"
    >
        <template #modal-header>
            <h5 v-if="typeProps == 'construction'">Создать договор подряда</h5>
            <h5 v-if="typeProps == 'supply'">Создать договор поставки</h5>
...
<script>
    ...
    export default {
        name: 'DirectoryContractsModalAdd',
        data() {
            return {
                form: {
                    specification: null,
        ...
        methods: {
            ...
            async newContract(){
                let form = this.form
                form.specification_id = this.form.specification.id
                form.partner_id = this.form.partner.id

                if(this.typeProps == 'construction'){
                    await this.directoryContractsWorkCreate(form)
                }

                if(this.typeProps == 'supply'){
                    await this.directoryContractsSupplyCreate(form)
                }
            ...
            hideModal(){
                this.$refs['directory-contracts-modal-add'].hide();
            },
            async mountedData(){
